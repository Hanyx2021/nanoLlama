...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0):
    ...
    freqs_cis = torch.concat([torch.cos(freqs).unsqueeze(dim=-1), torch.sin(freqs).unsqueeze(dim=-1)], dim=-1)
    return freqs_cis
...
def apply_rotary_emb(
    ...
    xk_ = xk.float().reshape(*xk.shape[:-1], -1, 2).to(freqs_cis.device)
    freqs_cis = reshape_for_broadcast(freqs_cis, xq_)

    xq_out = torch.concat([xq_[..., 0] * freqs_cis[..., 0] - xq_[..., 1] * freqs_cis[..., 1], 
                        xq_[..., 0] * freqs_cis[..., 1] + xq_[..., 1] * freqs_cis[..., 0]], dim=-1).flatten(3)
    xk_out = torch.concat([xk_[..., 0] * freqs_cis[..., 0] - xk_[..., 1] * freqs_cis[..., 1], 
